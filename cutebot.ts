...
namespace cuteBot {
    ...
    // === Support du simulateur ===
    let _simInitialized = false
    let _currentLeftSpeed = 0
    let _currentRightSpeed = 0
    let _currentHeadlightColor = 0
    ...
    function _sendSimState() {
        _initSim()
        cuteBot.sim.sendState(_currentLeftSpeed, _currentRightSpeed, _currentHeadlightColor)
    }
    // === Fin support simulateur ===
    ...
    */
    ...
    export function motors(lspeed: number = 50, rspeed: number = 50): void {
        ...
        // === Support simulateur ===
        _currentLeftSpeed = lspeed
        _currentRightSpeed = rspeed
        _sendSimState()
        // === Fin support simulateur ===
    ...
    */
    ...
    export function colorLight(light: RGBLights, color: number) {
        ...
        // === Support simulateur ===
        if (light === RGBLights.ALL || light === RGBLights.RGB_L) {
            _currentHeadlightColor = color
            _sendSimState()
        }
        // === Fin support simulateur ===
    ...
    */
    ...
    export function tracking(state: TrackingState): boolean {
        // === Support simulateur ===
        _initSim()
        cuteBot.sim.useSensor(cuteBot.sim.Sensors.LineDetector)
        
        if (cuteBot.sim.isSimulator()) {
            const lineState = cuteBot.sim.getLineState()
            if (lineState[0] >= 0) {
                const left = lineState[0]
                const right = lineState[1]
    ...
    */
    ...
    export function ultrasonic(unit: SonarUnit, maxCmDistance = 500): number {
        // === Support simulateur ===
        _initSim()
        cuteBot.sim.useSensor(cuteBot.sim.Sensors.Sonar)
        
        if (cuteBot.sim.isSimulator()) {
            const distance = cuteBot.sim.getSonarDistance()
            if (distance >= 0) {
                switch (unit) {
