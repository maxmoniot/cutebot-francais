...
namespace cuteBot {
    ...
    function _sendSimState() {
        _initSim()
        sim.sendState(_currentLeftSpeed, _currentRightSpeed, _currentHeadlightColor)
    }
    ...
    */
    ...
    export function tracking(state: TrackingState): boolean {
        // === Support simulateur ===
        _initSim()
        sim.useSensor(sim.Sensors.LineDetector)
        
        if (sim.isSimulator()) {
            const lineState = sim.getLineState()
            if (lineState[0] >= 0) {
    ...
    */
    ...
    export function ultrasonic(unit: SonarUnit, maxCmDistance = 500): number {
        // === Support simulateur ===
        _initSim()
        sim.useSensor(sim.Sensors.Sonar)
        
        if (sim.isSimulator()) {
            const distance = sim.getSonarDistance()
            if (distance >= 0) {
                switch (unit) {
